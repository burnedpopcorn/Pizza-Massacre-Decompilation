function scr_collide_monster()
{
    ...
    for (i = 0; i < t; i++)
    {
        if (!scr_solid_monster(x, y + (bbox_size_y * sv)))
        {
            y += (bbox_size_y * sv);
    ...
    sh = sign(hsp_final);
    down = scr_solid_monster(x, y + 1);
    
    for (i = 0; i < t; i++)
    {
        if (!scr_solid_monster(x + (bbox_size_x * sh), y) && down == scr_solid_monster(x + (bbox_size_x * sh), y + 1))
        {
            x += (bbox_size_x * sh);
        ...
        else
        {
            repeat (abs(target_x - x))
            {
                for (k = 1; k <= 3; k++)
                {
                    if (scr_solid_monster(x + sh, y) && !scr_solid_monster(x + sh, y - k))
                        y -= k;
                    
                    if (!scr_solid_monster(x + sh, y) && !scr_solid_monster(x + sh, y + 1) && scr_solid_monster(x + sh, y + (k + 1)))
                        y += k;
                }
                
                if (!scr_solid_monster(x + sh, y))
                {
                    x += sh;
                }
                else
    ...
    if (platformid != -4)
    {
        ...
        else
        {
            ...
            {
                if (scr_solid_monster(x, y))
                {
                    for (i = 0; scr_solid_monster(x, y); i++)
                    {
                        y--;
    ...
    onground |= (!place_meeting(x, y, obj_monsterplatform) && place_meeting(x, y + 1, obj_monsterplatform));
}
